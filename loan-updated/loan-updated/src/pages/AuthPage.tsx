...
import { ArrowRight, Smartphone, Chrome } from 'lucide-react';

// Simulated Google OAuth flow (in production, use @react-oauth/google or Firebase)
function simulateGoogleLogin(): Promise<{ id: string; name: string; email: string; picture: string }> {
...
export default function AuthPage() {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      const googleData = await simulateGoogleLogin();
            ...
            {step === 1 ? (
              <motion.div key="step1" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }}>
                ...
                <button
                  ...
                  className="w-full mt-6 h-11 px-6 rounded-xl font-bold transition-colors bg-white text-dark-900 hover:bg-dark-100 flex items-center justify-center gap-3 disabled:opacity-70"
                >
                  {googleLoading ? (
                    <span className="flex items-center gap-2"><span className="w-4 h-4 border-2 border-dark-400 border-t-brand-500 rounded-full animate-spin" /> Connecting to Google...</span>
                  ) : (
                    <>
                ...
                <Button 
                  type="button"
                  onClick={() => { login('9999999999'); navigate('/document-collection'); }}
                  variant="outline" className="w-full mt-4 text-brand-400 border-brand-500/30 hover:bg-brand-500/10"
                >
                  Quick Demo Login
                </Button>
              </motion.div>
              ...
              <motion.div key="step2" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }}>
                ...
                <form onSubmit={handleVerifyOtp} className="space-y-6">
                  <div className="flex justify-center gap-3">
                    <Input 
                      type="text" className="text-center tracking-[1em] font-mono text-xl"
