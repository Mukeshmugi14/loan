...
export default function DashboardLayout() {
  const { logout, calculateCompletion, user } = useAuth();
  ...
  return (
    <div className="min-h-screen bg-dark-950 flex flex-col md:flex-row">
      ...
      <aside className="hidden md:flex w-64 flex-col border-r border-dark-800/50 bg-dark-900/50 backdrop-blur-xl">
        ...
        <nav className="flex-1 px-4 py-6 space-y-1">
          {NAV_ITEMS.map((item) => (
            <NavLink
              ...
              to={item.path}
              className={({ isActive }) => 
                `flex items-center gap-3 px-3 py-3 rounded-xl transition-all font-medium border ${
                  isActive 
                    ? 'bg-brand-500/10 text-brand-400 border-brand-500/20' 
                    : 'text-dark-400 hover:bg-white/5 border-transparent hover:text-white'
                }`
              }
            >
              ...
              {item.path === '/profile/completion' && completion.overall < 100 && (
                <span className="ml-auto text-xs bg-brand-500/20 text-brand-400 px-1.5 py-0.5 rounded-full font-bold">
                  {completion.overall}%
                </span>
              )}
            </NavLink>
        ...
        {/* Profile Completion sidebar card */}
        <div 
          onClick={() => navigate('/profile/completion')}
          className="p-4 mb-4 mx-4 bg-gradient-to-br from-dark-800 to-dark-900 rounded-2xl border border-dark-700/50 cursor-pointer hover:border-brand-500/30 transition-all group"
        >
          <p className="text-xs text-dark-400 mb-1">Profile Completion</p>
          <p className="text-sm text-white font-semibold group-hover:text-brand-400 transition-colors">
            {completion.overall}% Complete
          </p>
          <div className="w-full bg-dark-950 border border-dark-850 h-1.5 rounded-full mt-2.5 overflow-hidden">
      ...
      <main className="flex-1 flex flex-col min-h-0 sm:pb-0 pb-20 md:pb-0 relative">
        <header className="h-16 sm:border-b border-dark-800/50 flex items-center justify-between px-4 sm:px-8 bg-dark-950/80 backdrop-blur-md sticky top-0 z-10 w-full">
          ...
          <h1 className="text-lg font-semibold text-white truncate sm:hidden">MSMERAISE</h1>
          <div className="flex items-center gap-4 shrink-0">
            ...
            {user?.profile_picture_url ? (
              <img src={user.profile_picture_url} className="w-8 h-8 rounded-full border border-dark-750 object-cover hidden sm:block" alt={user.full_name} />
            ) : (
              <div className="w-8 h-8 rounded-full bg-gradient-to-tr from-brand-500 to-blue-500 hidden sm:block"></div>
            )}
          </div>
        </header>
